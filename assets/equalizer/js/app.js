...
var App = {
	...
	initialize: function () {
		...
		$(document).on("submit",'#login_form_page', function (e){
			e.preventDefault();
	...
	events: function () {
		...
		for (i in App.events_time) {
			var html = '<li>';		
			html += '<p><span class="event-title">'+App.events_time[i].name+'</span><span class="time-start-event">'+App.events_time[i].time+'</span></p>';
			html += '<p><span class="starts-in">Starts In</span><span id="event'+App.events_time[i].id+'" class="time-to-event">'+App.formatedTime(App.events_time[i].left)+'</span></p>';
			html += '</li>';
			$("#events").append(html);
			i++;
		}
