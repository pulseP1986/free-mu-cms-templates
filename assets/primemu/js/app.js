...
var App = {
			...
			success: function (data) {
				if (data.error) {
					if(data.error == 'error_captcha'){
						...
						App.notice(App.lc.translate('Error').fetch(), 'error', App.lc.translate('Wrong Captcha').fetch(), 0);
	...
	arcaCountDown: function (id, end, start) {
		this.containerA = $('#' + id);
		this.endDateA = new Date(end * 1000);
		this.startDateA = new Date(start * 1000);
		if(new Date().getTime() > this.endDateA){
			this.containerA.html(App.lc.translate('Battle Ended').fetch());
		}
		else{
			var context = this,
				update = function () {
					context.startDateA.setSeconds(context.startDateA.getSeconds() + 1);
					var timediff = (context.endDateA - context.startDateA) / 1000;
					if (timediff < 0) {
						context.containerA.html(App.lc.translate('Battle Now').fetch());
					}
					else {
						context.containerA.html(App.formatedTime(timediff));
						setTimeout(update, 1000);
					}
				};
			update();
	...
	icewindCountDown: function (id, end, start) {
		this.containerA = $('#' + id);
		this.endDateA = new Date(end * 1000);
		this.startDateA = new Date(start * 1000);
		if(new Date().getTime() > this.endDateA){
